...
%   txt = [sprintf(a{1}), sprintf('*cos('), sprintf(num2str(freq(pks == max(pks)))), sprintf('*pi*x)+'), sprintf(b{1}), sprintf('*sin('), sprintf(num2str(freq(pks == max(pks)))), sprintf('*pi*x)')];
     ...
     for i = 2:min(n_season,length(freq))
    ...
    % txt = [sprintf(a{1}), sprintf('*cos('), sprintf(num2str(freq(1))), sprintf('*pi*x)+'), sprintf(b{1}), sprintf('*sin('), sprintf(num2str(freq(1))), sprintf('*pi*x)')];
    % for i = 2:n_season
    %     txt = [txt, sprintf('+'),sprintf(a{i}), sprintf('*cos('), sprintf(num2str(freq(i))), sprintf('*pi*x)+'), sprintf(b{i}), sprintf('*sin('), sprintf(num2str(freq(i))), sprintf('*pi*x)')];
    % end

    ft = fittype(txt, 'independent', 'x', 'dependent', 'y');
    opts = fitoptions('Method', 'NonlinearLeastSquares');
    opts.StartPoint = [repelem(0, min(n_season, length(freq))*2)];
    opts.Display = 'Off';



% elseif seasonality == "sine"
%     txt = 'a1*sin(b1*x+c1)';
