...
k     = size(R,2);
nk    = sum(R,1);
if any(nk < realmin)   % prevent division by zero
   nk = nk + eps;
end
w     = nk/sum(W);
...
[m, n] = size(U); % Dimensions of U (m rows, n columns)
[p, q] = size(X); % Dimensions of X (p rows, q columns)

%Check if the dimensions are compatible for U' \ X
if n ~= p
    disp('Dimension mismatch detected. Entering debug mode.');
    keyboard; % Pauses execution and enters debug mode
else
    % Proceed with the operation if dimensions are compatible
    Q = U' \ X;
end

% try
    % Operations that may cause errors
    Q = U' \ X;
 % catch ME
%     % Handle the error (log it, save intermediate results, etc.)
%     fprintf('Error in iteration %d: %s\n', ME.message);
%     % Optionally save the variables to check later
%     save(sprintf('debug_iter.mat'), 'U', 'X', 'mu', 'Sigma');
% end
