...
% [simY, x, ttm, v] = simulateY(par, par_names, T, ncontracts, nobsn, deltat, LT, correlation, serial, err)
...
% e_t ~ N(0,V_e)
% Assume AR(0) measurement errors
% For simplicity, we assume 260 days in a year, 30 days in a month.

function [simY, x, ttm, v, w] = simulateY_nG(par, model_options)
...
% rng(100);
n_par = size(par, 2);
par = set_parameters(LT, ncontracts, par_names, par, correlation, "no", err);
...
v = zeros(nobsn, ncontracts);
if err == "normal"
    v = mvnrnd(vnormmu, vnormsigma, nobsn);
elseif err == "laplace"
    v = randLaplace(vnormmu, vnormsigma, par.sG, nobsn);
elseif err == "hyperbolic"
    for i = 1:nobsn
        v(i,:) = randmgh(par.lambda, par.psi, par.chi, vnormmu, vnormsigma, par.nu')';
    end
end
