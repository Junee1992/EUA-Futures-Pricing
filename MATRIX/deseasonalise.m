function [season, periodicity, fitresult, gof] = deseasonalise(price, n_season, seasonality)
...
elseif seasonality == "sinusoid_b"
    txt = [sprintf(a{1}), sprintf('*cos('), sprintf(num2str(periodicity(1))), sprintf('*2*pi*x)+'), sprintf(b{1}), sprintf('*sin('), sprintf(num2str(periodicity(1))), sprintf('*2*pi*x)')];
    for i = 2:n_season
        txt = [txt, sprintf('+'),sprintf(a{i}), sprintf('*cos('), sprintf(num2str(periodicity(i))), sprintf('*2*pi*x)+'), sprintf(b{i}), sprintf('*sin('), sprintf(num2str(periodicity(i))), sprintf('*2*pi*x)')];
    end
    ft = fittype(txt, 'independent', 'x', 'dependent', 'y');
    opts = fitoptions('Method', 'NonlinearLeastSquares');
    opts.Display = 'Off';
