...
log_L = kf_v2_arp(par_init, par_names, y, deltat, ttm, LT, correlation, serial)
...
wt_1 = ks_v1(n_t, k, LT, par, deltat, ttm, att, att_1, Ptt, Ptt_1)
...
[simY, x, ttm, v] = simulateY(par, par_names, T, ncontracts, nobsn, deltat, LT, correlation, serial)

Simulates both state and measurement variables, along with time-to-maturities according to specified maturities.

Input:
- par -> set of parameters
- par_names -> the list of names of model parameters
- T -> a vector of maturities (in years); [1:5] corresponds to maturities in 1-5 years.
- ncontracts -> the number of available contracts in the price dataset
- nobsn -> the number of observations to be simulated
- deltat -> the time difference between each price. Set deltat = 1/260 for daily data.
- LT -> "GBM" if the long-term factor follows geometric Brownian motion.
     -> "OU" if the long-term factor follows Ornstein-Uhlenbeck process.
- correlation -> 0, assumes measurement errors are independent
...
Output:
- simY -> simulated price data
- x -> simulated state variables
- ttm -> simulated time-to-maturities
- v -> simulated non-stationary measurement errors